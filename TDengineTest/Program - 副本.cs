...
namespace TDengineDriver
{
    class TDengineTest
    {
        ...
        {
            PrintHelp(argv);
            host = this.GetArgumentAsString(argv, "-h", "192.168.1.101");
            user = this.GetArgumentAsString(argv, "-u", "root");
            password = this.GetArgumentAsString(argv, "-p", "taosdata");
            dbName = this.GetArgumentAsString(argv, "-d", "demo");
        ...
        public void ExecuteQuery()
        {
            ...
            sql1.Append("use ").Append(this.dbName);
            int code = TDengine.Query(this.conn, sql1.ToString());
            ...
            for (int i = 0; i < this.tableCount; ++i)
            {
                ...
                int fieldCount = TDengine.taos_field_count(conn);
                ...
                IntPtr rowdata;
                while ((rowdata = TDengine.taos_fetch_row(result)) != IntPtr.Zero)
                {
                    queryRows++;
                ...
                TDengine.taos_free_result(result);
            }
